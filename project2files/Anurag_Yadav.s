...
.text
	main:
			# receiving hexadecimal numbers from the user
			la $a0, input
			li $a1, 1001	
			li $v0, 8		
			syscall
			...
			li $s1, 0		# initializing the start pointer
			li $s2, 0		# initializing the end pointer
		find_words:
			la $s1, ($s2)
		find_single_word:
		...
		end_fw:
			...
			syscall
	subprogram_2:	
			la $s7, ($ra)		# Save the value of $ra to $s7
		...
		# Deletes space from the front of the string
			beq $t9, $t8, end_de	# Exits the loop
			add $t6, $t7, $t9	# $t6 points to the current character
			lb  $t5, ($t6)		
			# If space is found repeat the loop
			beq $t5, 32, found
			beq $t5, 9, found
			j delete_end		# Start deleting the space from the end if the current character is not a space
		found:
			addi $t9, $t9, 1
			j delete_front
		delete_end:
		...
		found1:
			...
			j delete_end
		end_de:
			beq $t9, $t8, not_number	# If there is no single character in the string, it is not a number
			li $t4, 0			# Initial decimal value
			li $s6, 0			# Length of the string
		convert:
		# check if the character is valid by calling the function
		...
		continue:
			...
			# Shift to the left by 4 bits to raise the power of 16
			sll $t4, $t4, 4
			sub $t6, $t5, $v1
			add $t4, $t4, $t6
			addi $s6, $s6, 1
			...
			j convert
		end_convert:
			bgt $s6, 8, too_large  # If the string of valid numbers is greater than 8,the string is too large to display
			li $v0, 1
			j end_s2
		too_large:
	...
	subprogram_1:
			...
			addi $v1, $0, 48			# store the ASCII dec to be subtracted to $s1
			blt $a0, 58, valid			#branch to if_invalid label if value in $t1 is less than 58 (next ASCII dec for number 9)		
			blt $a0, 65, invalid		# ASCII dec for 'A' = 65
			addi $v1, $0, 55
			blt $a0, 71, valid			# if $t1 ascii value is less than 97, head to invalid label
	...
	subprogram_3:
			...
			li $v0, 1
			mflo $a0
			beq $a0, 0, dont_print	# If the number is 0, it is not printed	
			syscall				
		dont_print:
			mfhi $a0
			syscall				
